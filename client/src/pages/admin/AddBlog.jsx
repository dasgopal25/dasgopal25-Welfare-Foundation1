...
export default function AddBlog() {
  ...
  const submit = async e => {
    ...
    } catch (err) { setError(err.response?.data?.message || 'Failed to create blog'); }
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="font-display text-2xl font-bold" style={{ color:'var(--c-primary)' }}>Add New Blog</h1>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div style={cardStyle}>
            ...
            <div className="space-y-4">
              <div><label style={labelStyle}>Title *</label><input name="title" value={form.title} onChange={handle} required placeholder="Blog title..." style={inputStyle} /></div>
              <div><label style={labelStyle}>Short Description *</label><textarea name="description" value={form.description} onChange={handle} required rows={3} placeholder="Brief description..." style={inputStyle} /></div>
              <div><label style={labelStyle}>Full Content * (HTML supported)</label><textarea name="content" value={form.content} onChange={handle} required rows={10} placeholder="Write full blog content..." style={inputStyle} /></div>
            </div>
          </div>
          <div style={cardStyle}>
            <h2 className="font-display font-semibold mb-4" style={{ color:'var(--c-primary)' }}>Content (বাংলা)</h2>
            <div className="space-y-4">
              <div><label style={labelStyle}>শিরোনাম</label><input name="titleBn" value={form.titleBn} onChange={handle} placeholder="বাংলা শিরোনাম..." style={inputStyle} /></div>
              <div><label style={labelStyle}>সংক্ষিপ্ত বিবরণ</label><textarea name="descriptionBn" value={form.descriptionBn} onChange={handle} rows={3} placeholder="সংক্ষিপ্ত বিবরণ..." style={inputStyle} /></div>
              <div><label style={labelStyle}>বিষয়বস্তু</label><textarea name="contentBn" value={form.contentBn} onChange={handle} rows={10} placeholder="সম্পূর্ণ বিষয়বস্তু লিখুন..." style={inputStyle} /></div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div style={cardStyle}>
            ...
            <div className="space-y-4">
              ...
              <div><label style={labelStyle}>Tags (comma-separated)</label><input name="tags" value={form.tags} onChange={handle} placeholder="welfare, education..." style={inputStyle} /></div>
          ...
          <div style={cardStyle}>
            ...
            <label className="block w-full cursor-pointer">
              <div className="w-full h-40 rounded-xl border-2 border-dashed flex items-center justify-center overflow-hidden transition-colors"
                ...
                {preview
                  ...
                  : <div className="text-center"><span className="text-3xl block mb-2">📸</span><p className="text-xs" style={{ color:'var(--c-text-muted)' }}>Click to upload</p></div>}
              </div>
